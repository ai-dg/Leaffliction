...
class Predictor:
    ...
    def predict(
        ...
        :return: Tuple of (predicted_label, probabilities_dict, transformed_images_dict).
    ...
    def infer_true_label(self, image_path: Path) -> Optional[str]:
        """
        Infer the ground-truth label from either the parent folder name or the filename.
        ...
        - Apple_Black_rot1, Apple_Black_rot_1, Apple_Black_rot (1), Apple_Black_rot-1
    ...
    def predict_with_dir(
        ...
        """
        Predict classes for all images in a directory (recursive if DatasetScanner does).

        Returns the same thing as `predict()` but for each image:
        { image_path: (predicted_label, probabilities_dict, transformed_images_dict) }

        :param dir_path: Path to directory containing images.
        :param model_zip: Path to model ZIP archive (optional).
        :param model_path: Path to model directory (optional).
        :param cfg: Prediction configuration.
        :return: Dict mapping image path to (predicted_label, probs, transformed)
        """
        ...
        for image_path, _true_id in items:
            ...
            self.logger.info(f"   True: {true_label} | Predicted: {predicted_label}")

            transformed = {}
            if cfg.show_transforms:
                transformed = self.transformation_engine.apply_all(img_resized)

            results[image_path] = (true_label, predicted_label, probs, transformed)
            self.logger.info("")
