...
class TrainConfig:
    ...
    Attributes:
        ...
        augment_train: If True, augmented images are included in the
            training set.
        transform_train: If True, transformed images are included in the
            training set.
...
class Trainer:
    ...
    def train(
            ...
            dataset_dir: Path,
            out_dir: Path,
            cfg: TrainConfig) -> Metrics:
        logger = self.logger
        index = self.dataset_scanner.scan(dataset_dir)
        logger.info(
            f"Dataset scan: Found {index.num_classes} classes, "
            f"{index.size} images")
        ...
        logger.info(
            f"Dataset split: Train: {len(train_items)} images, "
        ...
        if cfg.augment_train:
            ...
            logger.info(
                f"    Created {len(train_items)} total images "
                f"(original + augmented)")

        if cfg.transform_train:
            from leaffliction.transformations import TransformationDirectory
            ...
            train_items = self.transformation_engine.extract_transformed_items(
                train_items, transform_dir)
            valid_items = self.transformation_engine.extract_transformed_items(
                valid_items, transform_dir)
...
class ModelChecker:
    ...
    def assert_ok(self, metrics: Metrics) -> None:
        ...
        if metrics.valid_count < 100:
            ...
            exit()
        self.logger.info(
            f"   [OK] Validation set size: {metrics.valid_count} >= 100")
